...
import { sendBatchUpdate } from '@/lib/communication/actions'

export function BatchUpdateForm({ batches }: { batches: Array<{ id: string; name: string }> }) {
  const [state, action, pending] = useActionState(sendBatchUpdate, null)
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div>
        ...
        <textarea name="message" rows={4} required placeholder="Type your message here..." className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-900" />
      </div>
